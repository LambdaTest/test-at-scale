...
func TestTestBlockListService_GetBlockListedTests(t *testing.T) {
	...
	if err != nil {
		t.Errorf("New TestBlockListService object couldn't be initialized, error: %v", err)
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := tbs.GetBlockTests(tt.args.ctx, blYML, tt.args.repoID, tt.args.branch); (err != nil) != tt.wantErr {
