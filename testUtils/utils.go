...
import (
	...
	"github.com/LambdaTest/synapse/testUtils/testdata"
)
...
func GetConfig() (*config.NucleusConfig, error) {
	...
	configJSON, err := ioutil.ReadFile(cwd + testdata.ApplicationConfigPath)
	if err != nil {
		return nil, err
	}
...
func GetTaskPayload() (*core.TaskPayload, error) {
	cwd := getCurrentWorkingDir()
	payloadJSON, err := ioutil.ReadFile(cwd + testdata.TaskPayloadPath)
	if err != nil {
		return nil, err
	}
...
func GetPayload() (*core.Payload, error) {
	cwd := getCurrentWorkingDir()
	payloadJSON, err := ioutil.ReadFile(cwd + testdata.PayloadPath)
	if err != nil {
		return nil, err
	}
	var p *core.Payload
...
func GetGitlabCommitDiff() ([]byte, error) {
	cwd := getCurrentWorkingDir()
	data, err := ioutil.ReadFile(cwd + testdata.GitlabCommitDiff)
	if err != nil {
		return nil, err
	}
	return data, err
